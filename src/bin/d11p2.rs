...
fn occupied_neighbors(grid: &Grid<char>, x: usize, y: usize) -> usize {
    (-1..=1)
        ...
        .filter(|(ox, oy)| {
            let mut px = x as isize + ox;
            let mut py = y as isize + oy;

            while grid.in_bounds(px, py) {
                if *grid.get(px as usize, py as usize) == '#' {
                    return true;
                } else if *grid.get(px as usize, py as usize) == 'L' {
                    return false;
                }
