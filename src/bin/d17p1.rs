...
fn main() {
    ...
    let mut grid = Grid::new(
        input
            ...
            .map(|c| c == '#'),
        (0, 0, 0),
        (width as isize - 1, height as isize - 1, 0),
    );
...
fn advance(grid: &mut Grid<Coord>) {
    let (low, high) = {
        ...
        grid.ensure_coordinates(&(low.0 - 1, low.1 - 1, low.2 - 1));
        grid.ensure_coordinates(&(high.0 + 1, high.1 + 1, high.2 + 1));
        grid.get_extents()
    };
        ...
        .map(|c| (-1..=1)
            .flat_map(move |oz| (-1..=1)
                .flat_map(move |oy| (-1..=1)
                    .map(move |ox| (ox, oy, oz))
                )
            )
            .filter(|o| o.0 != 0 || o.1 != 0 || o.2 != 0)
            .filter(|o| grid.get(&(c.0 + o.0, c.1 + o.1, c.2 + o.2)))
            .count()
        )
        .collect::<Vec<_>>();
...
impl Coordinate for Coord {
    ...
    fn volume(min: &Self, max: &Self) -> usize {
        let d = (max.0 - min.0 + 1, max.1 - min.1 + 1, max.2 - min.2 + 1);
        (d.0 * d.1 * d.2) as usize
    }
